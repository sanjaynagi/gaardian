...
haps = allel.HaplotypeArray(haps).to_genotypes(ploidy=2).compress(female_bool, axis=1).compress(sibs_bool, axis=1)

assert metadata.shape[0] == haps.shape[1], "haplotypes and metadata are different shape"
...
random = random.query("population == @cohort")
haps = haps.to_haplotypes()
...
for i in np.arange(1, 201):
    ...
    alive_dead_dict['alive'] = random.query(f"r{i} == 'alive'").index.values
    alive_dead_dict['dead']  = random.query(f"r{i} == 'dead'").index.values

    assert random.iloc[alive_dead_dict['alive'], 'population'] == cohort, "pop does not equal cohort, is it indexing correctly?"
